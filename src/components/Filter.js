...
import product from "./smallProduct";
  ...
  const handleFlavorChange =  (e) => {
    ...
    if (selectedValue === "All") {
      ...
      const filtered = products.filter(product =>
        product.flavours && Array.isArray(product.flavours) && product.flavours.some(flavor => flavor === selectedValue)
);
  ...
  return (
    <div className="filter">
      <select className="flavorSelect" onChange={handleFlavorChange}>
        ...
        { products.map((product) => (product.flavours)).map((flavor) => (
